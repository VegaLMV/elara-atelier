...
export default function FilterSidebar({ categorias }: { categorias: Categoria[] }) {
    ...
    const updateFilter = (key: string, value: string) => {
        ...
        setIsOpenMobile(false); // Cierra el menú al seleccionar algo en móvil
    };
    ...
    // Este es el contenido principal de los filtros (reutilizable)
    const FilterContent = () => (
        <div className="space-y-12">
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                    <input
                        ...
                        className="w-full bg-transparent border border-[#e6dad1] rounded-sm px-3 py-2.5 text-sm text-[#3f2f2f] outline-none focus:border-[#864d2d] focus:ring-1 focus:ring-[#864d2d]/20 transition-all placeholder:text-[#3f2f2f]/30"
                    />
            ...
            {/* Categorías (Estilo Menú Editorial) */}
    ...
    return (
        <>
            ...
            {/* MOBILE DRAWER (Oculto por defecto, aparece al tocar el botón) */}
            {/* Overlay */}
            <div 
                className={cn(
                    "fixed inset-0 bg-[#3f2f2f]/40 backdrop-blur-sm z-50 lg:hidden transition-opacity duration-300",
                    isOpenMobile ? "opacity-100" : "opacity-0 pointer-events-none"
                )}
                onClick={() => setIsOpenMobile(false)}
            />
            
            {/* Panel Deslizante (Entra desde abajo) */}
            <div 
                className={cn(
                    "fixed bottom-0 left-0 right-0 h-[85vh] bg-[#fcfaf8] z-50 lg:hidden rounded-t-3xl shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] flex flex-col",
