...
export default function NavegacionClient({ initial }: { initial: NavItem[] }) {
  ...
  function move(id: string, dir: "up"|"down") {
    setItems(prev => {
      ...
      const merged = [
        ...
        ...other.map((x,i)=>({ ...x, order: i })), // se re-normaliza abajo
      ];
  ...
  const Section = ({ title, location, list }: { title: string; location: Location; list: NavItem[] }) => (
        ...
        {list.map((it) => (
          <div key={it.id} className="rounded-2xl border border-slate-200 p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                >
                  <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${
                    it.enabled ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-600"
                  }`}>
                    {it.enabled ? "ON" : "OFF"}
                  </span>
                </button>
              ...
              <div className="flex items-center gap-2">
                <button className="p-2 rounded-lg hover:bg-slate-100" onClick={() => move(it.id, "up")} title="Subir">
                  <ChevronUp className="w-4 h-4 text-slate-500" />
                </button>
                <button className="p-2 rounded-lg hover:bg-slate-100" onClick={() => move(it.id, "down")} title="Bajar">
                  <ChevronDown className="w-4 h-4 text-slate-500" />
                </button>
                <button className="p-2 rounded-lg hover:bg-red-50 text-red-500" onClick={() => del(it.id)} title="Eliminar">
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 bg-gray-50/50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Section title="Header" location="HEADER" list={header} />
        <Section title="Footer" location="FOOTER" list={footer} />
      </div>
    </div>
  );
}
