  ...
  async function guardar(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) { 
          ...
          setError(d?.error ?? "Error al registrar la compra"); 
          return; 
      }
       ...
       <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-5">
              ...
              <div className="space-y-1.5 relative">
                  ...
                  {provMenuOpen && (
                      <div className="absolute top-full left-0 w-full bg-white border border-gray-100 shadow-xl rounded-xl mt-2 max-h-48 overflow-y-auto z-50 p-1">
                          {proveedoresFiltrados.length === 0 ? (
                              <div className="p-3 text-xs text-gray-400 text-center">Sin resultados</div>
                          ) : (
                              proveedoresFiltrados.map(p => (
                                  <div key={p.id} 
                                       className="px-3 py-2 hover:bg-gray-50 cursor-pointer text-sm rounded-lg transition-colors font-medium text-gray-700 hover:text-slate-900"
                                               ...
                                               variantesFiltradas.map(v => (
                                                   <div key={v.id} className="p-2 hover:bg-gray-50 cursor-pointer rounded-lg flex items-center justify-between group transition-colors" 
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <span className="font-bold text-gray-800 text-sm block">{v.productoNombre}</span>
                                                                <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                                                                    <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 font-mono">{v.talla}</span>
                                                                    {v.colorHex && (
                                                                        <div className="flex items-center gap-1">
                                                                            <span className="w-3 h-3 rounded-full border border-gray-300 shadow-sm" style={{backgroundColor: v.colorHex}}></span>
                                                                            <span>{v.color}</span>
                                                                        </div>
                                                                    )}
                                                                </div>
                                               ...
                                               {variantesDelProducto.map(v => (
                                                   <tr key={v.id} className="hover:bg-blue-50/20 transition-colors">
                                                       <td className="px-4 py-2">
                                                           <div className="flex items-center gap-3">
                                                               {v.colorHex && <span className="w-3 h-3 rounded-full border border-gray-200 shadow-sm" style={{backgroundColor: v.colorHex}}></span>}
                                   ...
                                   <td className="px-4 py-3">
                                       <div className="flex flex-col">
                                           ...
                                           <div className="flex items-center gap-2 mt-1">
                                               ...
                                               {it.hexColor && <span className="w-2.5 h-2.5 rounded-full border border-gray-200 shadow-sm" style={{backgroundColor: it.hexColor}}></span>}
                                           </div>
                                       </div>
                                   </td>
